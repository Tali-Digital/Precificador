...
interface FormulaEditorModalProps {
  estaAberto: boolean;
  aoFechar: () => void;
  aoSalvar: (novaFormula: string) => void;
  titulo: string;
...
export const FormulaEditorModal: React.FC<FormulaEditorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-center p-4" onClick={manipularCliqueFundo}>
      <div className="bg-zinc-900 rounded-lg shadow-xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6">
          <label htmlFor="formula-editor" className="block mb-2 font-medium text-gray-300">
            Fórmula
          </label>
          <textarea
            id="formula-editor"
            ...
            className="w-full h-32 bg-black border border-zinc-700 rounded-md p-3 font-mono text-yellow-400 focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none transition"
            spellCheck="false"
          />
          <div className="mt-4">
            <h4 className="font-medium text-gray-300 mb-2">Variáveis disponíveis:</h4>
            <div className="flex flex-wrap gap-2">
              {variaveisDisponiveis.map((variavel) => (
                <span key={variavel} className="bg-zinc-700 text-gray-300 text-sm font-mono py-1 px-2 rounded">
                  {variavel}
                </span>
              ))}
            </div>
